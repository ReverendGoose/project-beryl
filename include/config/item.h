...
#define I_KEY_FOSSILS                   GEN_4  // In Gen4+, all Gen 3 fossils became regular items.
...
#define I_HEALTH_RECOVERY               GEN_4  // In Gen7+, certain healing items recover a different amount of HP than they used to.
#define I_SITRUS_BERRY_HEAL             GEN_4  // In Gen4+, Sitrus Berry was changed from healing 30 HP to healing 25% of Max HP.
#define I_VITAMIN_EV_CAP                GEN_4  // In Gen8+, the Vitamins no longer have a cap of 100 EV per stat.
...
#define I_GEM_BOOST_POWER               GEN_4  // In Gen6+, the Gem boost power was reduced from 50% to 30%.
...
#define I_TYPE_BOOST_POWER              GEN_4  // In Gen4+, all regular type boosting held items had their power increased from 10% to 20%. eg. Charcoal
#define I_SELL_VALUE_FRACTION           GEN_4  // In Gen9+, items sell for 1/4 of their value instead of 1/2.
#define I_PRICE                         GEN_4  // Some items have varied in value across generations.
#define I_BERRY_PRICE                   GEN_4       // Since Berries have become unplantable (Gen8+), their price has gone up.
#define I_POWER_ITEM_BOOST              GEN_4  // In Gen7+, Power Items grant 8 EVs instead of 4 EVs.
#define I_PREMIER_BALL_BONUS            GEN_4  // In LGPE onwards (Gen8+ here), you are given a Premier Ball for every 10 Poké Balls of any type and in the same purchase. Previously, it only applied to regular Poké Balls and only 1 could be obtained per purchase.
#define I_ROTOM_CATALOG_THUNDER_SHOCK   GEN_4  // In Gen9+, reverting Rotom to its base form will teach it Thunder Shock even if it knows another move.
...
#define I_EXP_SHARE_ITEM        GEN_4       // In Gen6+, the Exp. Share was changed from a held item to a Key item that toggles the effect described above.
...
// Fishing
#define I_FISHING_BITE_ODDS        GEN_4 // In Gen 1 and Gen 2, the Old Rod has a 100% chance for a bite, Good Rod has a 66% chance for a bite, and Super Rod has a 50% chance for a bite. In Gen 3, all rods have a base 50% chance for a bite. In Gen 4 onwards, the Old Rod has a base 25% chance for a bite, Good Rod has a 50% chance for a bite, and Super Rod has a 75% chance for a bite.
...
#define I_FISHING_ENVIRONMENT      GEN_4 // In Gen 3, the battle environment when fighting a hooked Pokémon is based on the tile the player is standing on. In Gen 4 onwards, the environment is based on tile that is being fished in, resulting in it usually being a water environment.
